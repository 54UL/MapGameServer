...
namespace MAP
{
    ...
    std::vector<uint8_t> NetInt::RawSerialization()
    {
    ...
    std::vector<std::shared_ptr<INetworkType>> NetInt::RawDeserialization(std::vector<uint8_t> argsMemory)
    {
        m_value = *reinterpret_cast<const int *>(argsMemory.data());
        return std::vector<std::shared_ptr<INetworkType>>() = {
            std::make_shared<MAP::NetInt>(m_value, "NULL")
        };
    }
